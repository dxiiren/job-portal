# Job Portal justfile — development recipes
...
port     := env_var_or_default('PORT', '8108')
...
bootstrap: _require-php _require-composer _require-node
    if (-not (Test-Path .env)) { Copy-Item .env.example .env; Write-Host "[INFO] Created .env from .env.example" -ForegroundColor Yellow }
