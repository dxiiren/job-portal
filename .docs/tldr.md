...
Browse a seeded catalog (100 jobs), filter by search / salary range / experience /
category, sign in (seeded accounts only — `akmal@gmail.com` / `password`), apply with an
...
private `local` disk. Seeding: 301 users / 20 employers / 100 jobs / random applications.
...
data, tests hit the dev sqlite, the length-based salary validation quirk.
...
`just fresh`), the out-of-the-box `just test` failure (`ExampleTest` expects 200, `/` is a
302), `just lint` failing on ~20 files of pre-existing Pint debt, missing PHP install,
unstyled pages (no Vite manifest), port 8108 conflicts and the multi-worker `php.exe`
shape, tests eating seeded data (phpunit DB overrides commented out), and the VC++/winget
UAC hang the setup script already guards against.
