...
For a scoped or careful commit ("commit only this", "commit the applications part"), ignore this
...
| `app/Http/Controllers/`, `app/Http/Requests/`, `routes/`   | `jobs` / `applications` / `employers` / `auth` |
| `resources/views/`, `resources/css/`, `resources/js/`      | `views` / `ui`     |
...
scope: jobs, applications, employers, auth, models, db, views, ui, config, test, build, docs, skills
```
...
- Ported from the book-review pilot for job-portal (Laravel 12). Scope table remapped to
  this repo's domains (jobs / applications / employers / auth); hook-failure handling stays
  removed (no pre-commit hooks); build artifacts list unchanged (`vendor/`, `public/build/`,
  sqlite db).
