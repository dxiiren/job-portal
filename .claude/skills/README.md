# Skills Catalog — `job-portal`

Project development skills for this Laravel 12 / Blade / SQLite job-portal app (GitHub, local-only). Each lives in its own directory with a `SKILL.md`. **Follow the relevant skill before writing code.** Run `/audit-skills` to verify every skill here is registered and that `CLAUDE.md` references only existing skills.
...
| Skill                                   | What it does                                                                                                                      | Model  |
| --------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | ------ |
| [pre-pr-review](pre-pr-review/SKILL.md) | Self-review the branch diff against a Laravel / Eloquent / Blade / security checklist + the quality suite; report to `workspace/reports/pr/`. | opus   |
...
## Stack add-ons not included (rationale)

Per the onboarding kit, optional add-on skills ship only when their prerequisite exists in
the repo. None do here:

| Skill | Why absent |
| --- | --- |
| `fix-typecheck` | No TypeScript / typecheck script — Blade views + a 6-line Alpine boot in `resources/js`. |
| `generate-playwright-tests` | No Playwright dependency or config committed. |
| `fix-phpstan` | phpstan is not in `composer.json` (quality gate is Pint + PHPUnit). |
| `monitor-ci` | No CI workflow files — this repo runs locally only. |
