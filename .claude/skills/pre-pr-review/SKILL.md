...
| --- | --------------------------- | ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **Validation**              | issue      | Controller reading `$request->input()` and persisting without a FormRequest (`JobRequest` / `StoreJobApplicationRequest` are the house pattern); missing `rules()` for a new field; a numeric field validated without `numeric`/`integer` (min/max then check string LENGTH — the historic `expected_salary` bug, fixed and guarded by `JobApplicationSalaryValidationTest`).       |
