...
specialist / a Blade accessibility reviewer}. {Any relevant seniority, domain, or mindset.}
...
situation: what the project is (a Laravel 12 job-portal app: job listings, employer accounts,
CV-upload applications), the domain, and WHY this task matters. Self-contained - assume the
reader has never seen this codebase.}
...
## 6. Constraints & Rules
- Tech stack: {Laravel 12 / PHP 8.4 / SQLite / Blade / PHPUnit 11, versions}
...
````text
# MASTER PROMPT - Fix expected-salary validation accepting non-numeric input

## 1. Role
You are a senior Laravel engineer who writes precise, minimal fixes.

## 2. Mission
Make the job-application form reject non-numeric expected salaries while keeping the
1..1,000,000 bounds check, and explain the root cause in one paragraph.

## 3. Background & Context
Small Laravel 12 job-portal app. Signed-in users apply to a job with an expected salary and
a PDF CV. The store endpoint validates via a FormRequest. Submitting expected_salary="abc"
passes validation and blows up on the integer DB column. No repo access - everything you
need is below.

## 4. Current State
- Done: numeric salaries in range save fine.
- Tried: reordering the rules - no effect.
- Failing: expected_salary="abc" passes validation, then the insert throws.
...
## 6. Constraints & Rules
- Laravel 12 validation rules. Keep the cv file rule untouched.

## 7. Your Task
Return the corrected rules() array and a one-paragraph root-cause explanation (hint:
without a numeric/integer rule, min/max validate string LENGTH, not value).

## 8. Output Format & Success Criteria
- Output: the fixed rules() only + the explanation.
- Done: 1..1000000 accepted, "abc"/0/1000001 rejected, cv rule still enforced.

## 9. Open Questions & Assumptions
- Assume the form posts expected_salary as a string ("45000") - note if that matters.

## Reminder
Return the fixed rules() + a one-paragraph root cause. Keep the cv rule untouched.
````
...
- Ported from the book-review pilot for job-portal (Laravel 12). Personas and stack
  constraints kept; the worked example rewritten around this repo's
  `StoreJobApplicationRequest` salary-validation shape.
