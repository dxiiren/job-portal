...
- {Step to bring the world up + how to check it's up + what to do if it's down. e.g. `just
  bootstrap` if `vendor/` is missing, the dev server (`just start` on :8108), a sanity gate
...
```markdown
# GOAL - Backfill feature tests for the three job-portal endpoints
...
Write feature tests so `jobs.index` (search + salary/experience/category filters), `jobs.show`,
and `job.application.store` (validation + CV upload + applyJob policy) each have passing
...
- Use model factories (`JobFactory`, `EmployerFactory`, `JobApplicationFactory`) - never
  hand-inserted rows.
...
- Real: an endpoint depends on a service with no test double and no reasonable fake -
  cite the `file:line`.
- Not a blocker: "the filter logic is complex" - slice the cases and cover them.

## Definition of Done

Per item - `DONE` only when:

- [ ] The endpoint's test covers every branch (filters / validation errors / policy denials)
...
| 1   | JobIndexTest                | `TODO` |
| 2   | JobShowTest                 | `TODO` |
| 3   | StoreJobApplicationTest     | `TODO` |
