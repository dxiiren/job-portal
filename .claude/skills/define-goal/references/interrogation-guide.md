...
- What's the before->after transformation, stated concretely enough to apply uniformly (e.g.
  inline controller queries -> model scopes, or a Blade component extraction)?
...
- This repo has **no deployment target** - it runs locally only. A "deployment" goal here means
  local serving (`just start` on :8108) or authoring future deploy docs. If the developer wants a
  real deploy, first interrogate WHERE (host, method) - that decision is itself part of the goal.
- What's the health-verify that proves success (an HTTP 200 from `curl.exe` on :8108, a migration
  run, a seeded page rendering)?
