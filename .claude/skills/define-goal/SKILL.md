...
- Ported from the book-review pilot for job-portal (Laravel 12). Examples repointed at this
  repo's endpoints (jobs index/show, job applications) on the same Laravel/just stack
  (`just bootstrap`/`just test`); local-only reality unchanged.
