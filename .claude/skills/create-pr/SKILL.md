...
- If there are multiple commits, synthesize one Conventional title that summarizes
  the change, e.g. `feat(jobs): add salary-range filter to job list` or
  `fix(applications): block duplicate application on re-submit`. Pick the type/scope from the
...
- <how it was verified: `just test` result, manual check on http://127.0.0.1:8108, etc.>
```
...
| `/pre-pr-review` | Optional self-review (Laravel/Blade checklist) before opening the PR          |
