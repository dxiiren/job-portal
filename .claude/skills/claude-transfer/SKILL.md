...
5. **Review in plan mode**, then on approval (create the folder if missing):
   - Write `.claude/workspace/reports/transfers/claude/{YYYY-MM-DD}-{topic}.md`, where
     `{topic}` names **what the session did** (e.g. `add-job-salary-filter`), **not** this
     skill's name.
...
3. **Re-ground before acting** (do NOT start new work first): read the brief's pointers + the
   ...
   and load http://127.0.0.1:8108/jobs) to catch drift or undocumented breakage. Building a
...
- Files: `app/Http/Controllers/JobController.php:13-31`, `app/Models/Job.php`
...
- Dev server on :8108 was {up/down} at close - re-check (`just start` / `just stop`)
...
- Ported from the book-review pilot for job-portal (Laravel 12). Commands + freshness checks
  kept on the Laravel/just stack (`just start` on :8108, `just test`, migrate status);
  examples repointed at JobController / Job.
